...
void main() {
  ...
  test("Not sending a reply to a message in time should receive abort from remote", () async {
    // Test for exception generated by clientChannel
    // not necessary serverChannel.outgoingSafety = false;
    expect(() async {
      await serverChannel.sendRequest("HALO", new Uint8List(0));
    }(), throwsA(isInstanceOf<TalkAbort>()));
  });

  test("Not receiving a reply to a message in time should abort locally", () async {
  ...
  test(
      "Can send basic request messages and receive response",
      () async {
    Uint8List sentPayload = Uint8List.fromList([
      random.nextInt(256),
      random.nextInt(256),
